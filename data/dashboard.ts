...
export interface DashboardSummaryResponse {
  ...
  kepala_riset_metrics: {
    total_documents: number;
    annotated_documents: number;
    total_reviewed_annotations: number;
...
export interface IAAResponse {
  ...
  detailed_analysis: {
    ...
    perfect_agreement_spans: Array<[number, number, number, number]>;
    annotator_only_spans: Array<[number, number, number, number]>;
    reviewer_only_spans: Array<[number, number, number, number]>;
  };
  metadata: {
    execution_time: number;
    data_size: number;
