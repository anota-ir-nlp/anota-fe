    ...
    <div class="w-full max-w-[95vw] mx-auto px-2 sm:px-4 pb-16">
      ...
      <div class="grid gap-6" :class="getGridLayout()">
        ...
        <div v-if="currentViewMode === 'combined'" class="space-y-6">
          ...
          <!-- Section 2: All Sentences as Paragraph, Annotation Controls at Top -->
          <div class="bg-white border border-gray-200 rounded-3xl p-6">
            <!-- Annotation controls and info at the top when a sentence is selected -->
            <div v-if="selectedSentence" class="mb-4 flex flex-col gap-2">
              <div class="flex flex-wrap items-center gap-2 justify-between">
                <span
                  class="px-2 py-1 rounded-full bg-blue-500 text-white text-xs font-semibold"
                >
                  Kalimat Terpilih #{{ selectedSentence.id }}
                </span>
                ...
                <span
                  ...
                  v-bind="
                    selectedSentence && selectedSentence.id === sentence.id
                      ...
                      : {}
                  "
                  @mousedown="onSentenceMouseDown(sentence)"
                  @mouseup="
                    selectedSentence && selectedSentence.id === sentence.id
                      ? handleTextSelection
                      : undefined
...
const sentenceSpans = ref<HTMLElement[]>([]);
...
// Remove lastMouseDownSentenceId logic for this section, use contenteditable and selection logic like [id] copy.md
...
// Ensure keyboard shortcuts work when editableArea is focused
...
// Optional: keep editableArea content in sync with original sentence (prevent editing)
function onEditableInput(e: Event) {
  if (!selectedSentence.value) return;
  // Reset content to original sentence (prevent user editing)
  (e.target as HTMLElement).innerText = getOriginalSentenceText(selectedSentence.value.id);
}
...
function handleSentenceKeydown(e: KeyboardEvent) {
  // Always allow keyboard shortcuts when sentence is selected
  if (e.key === "Enter" && !e.shiftKey) {
    e.preventDefault();
    if (canAnnotate.value) showAnnotationUI.value = true;
  }
...
.sentence-row[aria-disabled="true"] {
  user-select: none;
  pointer-events: none;
}
.sentence-row[aria-disabled="true"] .sentence-text {
  color: #a1a1aa !important;
  background: #f3f4f6 !important;
  border-color: #e5e7eb !important;
}
.sentence-row .sentence-text {
  transition: background 0.2s, color 0.2s, border 0.2s;
}
.sentence-row .sentence-text:focus {
  outline: 2px solid #2563eb;
}
.sentence-row .sentence-index {
  font-family: monospace;
}
.sentence-row[aria-disabled="true"] .sentence-index {
  color: #d1d5db !important;
}
.paragraph-annotasi {
  /* Make sure sentences flow like a paragraph */
  display: block;
  line-height: 2;
  font-size: 1.08em;
}
...
.sentence-in-paragraph.active-sentence {
...
.sentence-in-paragraph.disabled-sentence {
  background: #f3f4f6 !important;
  color: #a1a1aa !important;
  pointer-events: none !important;
  user-select: none !important;
}
.sentence-in-paragraph:focus {
  outline: 2px solid #2563eb;
}
.sentence-in-paragraph:hover:not(.disabled-sentence):not(.active-sentence) {
  background: #e0e7ff !important;
}
.sentence-in-paragraph .ukbd,
.sentence-in-paragraph .UKbd {
  display: inline-block;
  margin-left: 0.25em;
}
